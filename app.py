...
def load_kunjungan():
    try:
        res = supabase.storage.from_("laporan_files").download("kunjungan.json")
        return json.loads(res)["total"]
    except:
        return 0
        
def save_kunjungan(total):
    data = json.dumps({"total": total}).encode('utf-8')
    try:
        supabase.storage.from_("laporan_files").update(path="kunjungan.json", file=data)
    except:
        supabase.storage.from_("laporan_files").upload(path="kunjungan.json", file=data)
...
# ==========================================
    ...
    if not st.session_state['sudah_login']:
        ...
        # --- TAB 1: DASHBOARD EKSEKUTIF (SATU HALAMAN PENUH) ---
        with tab1:
            # 1. FILTER DASHBOARD
            f1, f2, f3 = st.columns(3)
            with f1:
                dash_tahun = st.selectbox("📅 Periode Tahun:", DAFTAR_TAHUN, index=2)
            with f2:
                dash_jenis = st.selectbox("📋 Kategori Laporan:", ["Bulanan", "Tahunan"])
            with f3:
                if dash_jenis == "Bulanan":
                    dash_bulan = st.selectbox("📆 Periode Bulan:", ["Januari", "Februari", "Maret", "April", "Mei", "Juni", "Juli", "Agustus", "September", "Oktober", "November", "Desember"])
                else:
            ...
            # PENGAMBILAN DATA KUNJUNGAN
            total_kunjungan = load_kunjungan()
            
            # 2. BARIS KOTAK METRIK UTAMA
            m1, m2, m3 = st.columns(3)
            m1.metric("👥 Total Kunjungan Pasien", f"{total_kunjungan:,}")
            m2.metric("🎯 Persentase Kepatuhan Lapor", f"{persen_patuh}%")
            m3.metric("📂 Unit Telah Melapor", f"{jml_sudah} dari {total_p} Program")
            
            st.write("")
            st.write("")
            
            # 3. BARIS GRAFIK (KIRI & KANAN)
            col_kiri, col_kanan = st.columns(2)
            
            with col_kiri:
                st.markdown("<div class='kepatuhan-card'>", unsafe_allow_html=True)
                st.subheader("📋 Status Kepatuhan Unit")
                st.markdown("<hr style='margin: 10px 0;'>", unsafe_allow_html=True)
                st.write("**✅ Sudah Menyerahkan Laporan:**")
                if jml_sudah > 0:
                ...
                st.write("")
                st.write("**⏳ Belum Menyerahkan Laporan:**")
                if len(program_belum) > 0:
                    st.markdown("".join([f"<span class='badge-belum'>⏳ {p}</span>" for p in program_belum]), unsafe_allow_html=True)
                else:
                    st.success("Luar biasa! Semua unit sudah lapor bulan ini.")
                st.markdown("</div>", unsafe_allow_html=True)
                
            with col_kanan:
                st.markdown("<div class='kepatuhan-card'>", unsafe_allow_html=True)
                st.subheader("🦠 10 Besar Penyakit Teratas")
                st.markdown("<hr style='margin: 10px 0;'>", unsafe_allow_html=True)
                ...
                if not df_penyakit.empty:
                    ...
                    st.info("Belum ada data penyakit. Silakan update melalui pengaturan di bawah.")
                st.markdown("</div>", unsafe_allow_html=True)
                
            st.write("")
            
            # 4. KONTROL ADMIN (DISIMPAN DI DALAM EXPANDER AGAR RAPI)
            with st.expander("⚙️ Pengaturan Dashboard (Update Angka & Grafik)"):
                upd_c1, upd_c2 = st.columns(2)
                
                with upd_c1:
                    st.write("**Ubah Total Kunjungan**")
                    with st.form("form_kunjungan"):
                        new_kunjungan = st.number_input("Masukkan total kunjungan baru:", value=total_kunjungan, min_value=0)
                        if st.form_submit_button("💾 Simpan Angka Kunjungan", use_container_width=True):
                            save_kunjungan(new_kunjungan)
                            st.success("Angka kunjungan diperbarui!")
                            st.rerun()
                            
                with upd_c2:
                    st.write("**Update Grafik 10 Besar Penyakit**")
                    file_p = st.file_uploader("Upload Excel '10 Besar Penyakit'", type=['xls', 'xlsx'])
                    if st.button("🔄 Proses File Excel", use_container_width=True):
                        if file_p is not None:
                            try:
                                ...
                                if "NAMA PENYAKIT" in df_raw.columns and "JML" in df_raw.columns:
                                    ...
                                    st.success("✅ Grafik penyakit diperbarui!")
                                    st.rerun()
                                else:
